...
> **【强制门控】发布必须由用户手动触发，工具不得自行发布**
>
> 1. 每次代码 / 功能改动完成后，先完成「构建与验证」章节的完整构建链与单元测试；
> 2. 构建验证通过后**立即暂停**，不进入下方发布步骤，将构建产物与验证结果交给用户；
> 3. 由用户打开新的 `dist\L.Note.exe` 在本地手动验证 / 测试本次改动；
> 4. **仅当用户手动输入「发布」后**，才允许执行下方第 1–4 步发布流程。
>
> 任何情况下，工具在用户显式输入「发布」之前，都不得执行远程推送、修改 CHANGELOG / 功能列表、更新 GitHub Pages、创建 GitHub Release 等发布动作。

用户手动验证通过并输入「发布」后，**必须**按以下顺序执行，缺一不可：
...
注意：`site/` 目录已废弃（仅残留与根目录相同的 `site/index.html` 副本），**发布流程不再读取** **`site/`**。需要更新的文件（均在 master 根目录）：

- `download.html`：版本号、下载链接（GitHub Releases）、校验和命令（`certutil -hashfile`）；改完后执行 `Copy-Item download.html index.html` 保持 `index.html` 为同一副本

- `index.html`：与 `download.html` 内容一致的首页副本（发布源，非部署时生成）

- `blink-guide.html`：顶部 badge 版本号

- `RELEASE-NOTES.md`：发布说明（版本标题、下载文件名、本版亮点、历史版本说明）

- `SHA256SUMS.txt`：用 `certutil -hashfile dist\L.Note.exe SHA256` 计算新 exe 的校验和并替换

- `resource/`：app 图标（favicon / logo 引用 `resource/app_32.png`、`resource/app_512.png`），通常无需改动，但**必须随发布一并检出，不得遗漏**

- `screenshots/`：仅当截图有更新才需要改动

提交 master 并完成第 1 步推送后，将上述文件从 master 检出到 gh-pages **根目录**并推送 GitHub：

```bash
git checkout gh-pages
git rm -r -q .                                   # 清空 gh-pages 根目录旧发布文件
git checkout master -- .gitignore                # gh-pages 无 .gitignore，必须恢复以防 git add 误加
git checkout master -- download.html
git checkout master -- index.html
git checkout master -- blink-guide.html
git checkout master -- RELEASE-NOTES.md
git checkout master -- SHA256SUMS.txt
git checkout master -- resource                  # 必须！图标/favicon 来源；漏掉会导致线上图片全丢（30b91c9 教训）
git checkout master -- screenshots               # 必须！git rm 已清空，即使截图未改动也要重新检出
git add -A
# —— 防回退校验（30b91c9 曾漏检 resource/ 致线上图标全丢）：未检出齐 6 个图标则中止，禁止提交 ——
$n = (git diff --cached --name-only -- resource | Measure-Object -Line).Lines
if ($n -lt 6) { throw "resource/ 未完整检出（$n 个文件），禁止提交 gh-pages" }
git commit -m "chore(site): 更新发布页面 vX.Y.Z"
git push github gh-pages
git checkout master
```

发布后验证：用 `Invoke-WebRequest "https://stutasliu.github.io/LNote/?cb=<时间戳>"`（带缓存绕过参数）核对页面出现 `vX.Y.Z`，且 `download.html` 中 `<link rel="icon" ... resource/app_32.png>` 与 logo 图片 `resource/app_512.png` 均可访问（HTTP 200）；若仍旧，先确认 `GET /pages/builds/latest` 的 `status=built` 且 commit 为刚推送的哈希，再等 CDN 缓存过期（约数分钟）后重试，勿在构建完成前反复断言。
...
1. **布局/样式问题要考虑所有状态**：不能只针对当前暴露的场景打补丁。同一元素在不同状态（显示/隐藏、不同文档类型、不同窗口宽度）下行为不同，修复必须让"所有状态"都正确，避免修 A 坏 B。
...
2. **一处引用的调用链自查**：改动函数/变量/元素时，检查所有调用方、守卫条件（如 `if (els.xxx)`）、以及是否还有其他代码依赖旧行为。
3. **新增功能必须接入全链路**：新功能（尤其新增文档类型 / 面板 / 格式化命令 / 右键菜单 / 快捷键 / 状态持久化）要枚举所有相关守卫与入口并逐一接入，漏一处即出错。

   - 真实教训：新增 PDF/Word 的 `kind` 时，需在保存 / 导出 / 磁盘写回 / 复制全链路拦截只读，漏一处就出现"能改动只读文档"；文档地图需覆盖"仅文本文档显示 / 非文本自动隐藏 / 开关状态持久化"等多个分支。
4. **考虑边界与极端场景，并回归已有功能**：剪贴板多格式（文本+图片）要文本优先、半截 / 混合 JSON 不误判、空文档 / 空 PDF 告警、超大文档首屏不卡顿、启动默认文档不闪现；同时验证改动不影响既有功能。
