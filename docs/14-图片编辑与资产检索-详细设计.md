...
本文档对应 L.Note 富文档的图片能力增强，覆盖两个阶段：Phase 1 为图片编辑，Phase 2 为图片资产检索。文档面向前端工程师与设计评审，描述系统的模块划分、接口契约、数据结构与迁移路径，不涉及逐行代码实现。
...
graph TB
    subgraph "Python 原生层 main.py"
        IA["LNoteApi<br/>scan_image_library(新增)<br/>save_image_binary(复用)"]
...
| 标识 | 位置 | 操作 | 状态 |
|------|------|------|------|
| I1 | Python LNoteApi | `save_image_binary(dirpath, filename, content_b64, subdir)` | 复用，不改签名 |
| I2 | Python LNoteApi | `scan_image_library(root, max_depth)` | 新增 |
| I3 | Python LNoteApi | `open_image_viewer_window(src, name, base_dir)` | 修改，加第三参数 |
...
| 文件 | 改动 | 风险 |
|------|------|------|
| main.py | LNoteApi 增 `scan_image_library`；ImvApi 增 `save_image_binary`；`open_image_viewer_window` 与 `get_image` 扩展字段 | 低，纯增量 |
...
| css/lnote-rich.css | 编辑面板与检索面板样式 | 低 |
